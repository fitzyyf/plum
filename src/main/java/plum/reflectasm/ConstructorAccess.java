...
public abstract class ConstructorAccess<T> {
	...
	public static final String CONSTRUCTOR_ACCESS_LOCATION = "plum/reflectasm/ConstructorAccess";
		...
		synchronized (loader) {
			try {
				...
				cw.visit(V1_1, ACC_PUBLIC + ACC_SUPER, accessClassNameInternal, null,
						CONSTRUCTOR_ACCESS_LOCATION, null);
	...
	static private void insertConstructor (ClassWriter cw) {
		...
		mv.visitMethodInsn(INVOKESPECIAL, CONSTRUCTOR_ACCESS_LOCATION, "<init>", "()V");
		mv.visitInsn(RETURN);
		mv.visitMaxs(1, 1);
		mv.visitEnd();
	}
